...
description: "PostgreSQL + Apache AGE 后端，可在标准 SQL 之外运行 openCypher 查询。"
icon: "database"
---
...
Apache AGE 是一个为 PostgreSQL 添加图数据库功能的扩展，让你可以在传统 SQL 之外运行 openCypher 查询。这一后端让 Semantica 能够把 AGE 当作属性图存储来使用，且接口与 Neo4j、FalkorDB 完全相同。
...
<Note>Apache AGE 必须编译并安装到你的 PostgreSQL 实例中。参见 [AGE 安装指南](https://age.apache.org/age-manual/master/intro/setup.html)。</Note>
...
<Tabs>
  ...
  <Tab title="直接使用（ApacheAgeStore）">
    当你需要 AGE 特有的行为时，可以直接导入 `ApacheAgeStore`：
...
在 `connect()` 时，存储会执行幂等的初始化设置：可以安全地重复调用：
...
Apache AGE 会自动生成内部的顶点/边 ID（很大的整数）。这些 ID **不同于**你可能想指定的任何语义层或应用层 ID。

| 概念 | 说明 |
| :--------- | :------------- |
| **AGE 内部 ID** | 由 AGE 自动生成，在所有返回的字典中以 `"id"` 暴露，用于 `delete_node()`、`get_node()` 等。 |
| **语义 ID** | 应用层标识符，保存在 `semantica_id` 属性中。 |
...
<Warning>切勿把 AGE 内部 ID 与语义 ID 混用。图操作（删除、更新、遍历）请使用 `node["id"]`，应用层查找请使用 `node["properties"]["semantica_id"]`。</Warning>


## 标签处理

AGE 严格限制**每个顶点只能有一个标签**。Semantica 会透明地处理这一点：

- `labels[0]` → 作为 AGE 顶点的主标签。
- `labels[1:]` → 存储在顶点的 `labels` 属性数组中。
...
所有 Cypher 查询都通过 AGE 的 SQL 包装层执行：
...
AGE 不支持在 `cypher()` 调用中使用 `$param` 风格的绑定。存储会安全地把参数转换为经过正确转义的 Cypher 字面量：
...
若省略该选项，存储会尝试从 `RETURN` 子句推断列。


## 事务

存储使用显式的 PostgreSQL 事务：
...
所有方法都与 Semantica 标准图存储后端接口一致：

| 方法 | 说明 |
| :-------- | :------------- |
| `connect(**options)` | 连接并初始化 AGE |
| `close()` | 关闭连接 |
| `create_node(labels, properties)` | 创建顶点 |
...
| `delete_node(node_id, detach)` | 删除顶点 |
| `create_relationship(start_id, end_id, type, properties)` | 创建边 |
| `get_relationships(node_id, rel_type, direction, limit)` | 查询边 |
| `delete_relationship(rel_id)` | 删除边 |
| `execute_query(query, parameters)` | 执行任意 Cypher |
| `get_neighbors(node_id, rel_type, direction, depth)` | 图遍历 |
| `shortest_path(start_id, end_id, rel_type, max_depth)` | 路径查找 |
